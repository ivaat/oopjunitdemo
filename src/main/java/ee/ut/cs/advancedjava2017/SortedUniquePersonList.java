 ...
 * 2) Elements are guaranteed to have unique ID code values.<br><br>
 *
 * Uses an underlying array for storing the elements. <br>
 * The object guarantees to not use more than twice the required array size, unless {@link #forceSetCapacity(int) forceSetCapacity} has been invoked.<br>
...
public interface SortedUniquePersonList {
    ...
    /**
     * Returns the index of the person matching the given object. If matching object is not present, returns -1.<br>
     * All significant fields (both names + ID) must match exactly for the objects to match.
     * @param person Person that is searched.
     * @return Index at which the given person object can be found, or -1 if no such person is present.
     */
    ...
    /**
     * Returns the current capacity of the underlying array, according to the general contract described in class docstring.<br>
     * For empty objects, including those newly created, capacity is guaranteed to be 0.
     * @return Current capacity (size of the underlying array).
     */
    public int capacity();

    /**
     * Forces the size of the underlying array to be set to the given value. Can be used for memory optimization if known that list elements will not be modified further, or if it is known that many elements will be added to this list sequentially.
     * @param capacity New size of the underlying array.
     * @throws Exception If capacity cannot be set to the given value, for example if actual size is larger than forced capacity.
     */
